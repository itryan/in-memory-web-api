...
export var InMemoryBackendService = (function () {
    ...
    InMemoryBackendService.prototype.commands = function (reqInfo) {
        ...
        switch (command) {
            ...
            case 'config':
                ...
                break;
            default:
                resOptions = this.createErrorResponse(STATUS.INTERNAL_SERVER_ERROR, "Unknown command \"" + command + "\"");
        }
        return this.createObservableResponse(resOptions);
    };
    ...
    InMemoryBackendService.prototype.get = function (_a) {
        ...
        if (!data) {
            return this.createErrorResponse(STATUS.NOT_FOUND, "'" + collectionName + "' with id='" + id + "' not found");
        }
        return new ResponseOptions({
    ...
    InMemoryBackendService.prototype.put = function (_a) {
        ...
        if (id !== item.id) {
            return this.createErrorResponse(STATUS.BAD_REQUEST, "\"" + collectionName + "\" id does not match item.id");
        }
        var existingIx = this.indexOf(collection, id);
        if (existingIx > -1) {
            collection[existingIx] = item;
